...
import uvicorn
import time
from aithelper import aitemplate_sd, gen_img
import threading 
...
def get_image(
    ...
    if(not ("tmp_"+str(width)+"_"+str(height)+"_"+str(batch_size) in listdir)):
        ...
        image = helper.get_image(prompt,height,width,num_inference_steps,guidance_scale,negative_prompt,batch_size)
...
#     image = helper.pipe_nsd,prompt,height,width,num_inference_steps,guidance_scale,negative_prompt).result().images
#     if(len(image)==1):
#         filtered_image = io.BytesIO()
#         image[0].save(filtered_image, "JPEG")
#         filtered_image.seek(0)
